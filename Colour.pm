...
require Exporter;
@ISA = qw(Exporter);
@EXPORT   = qw(&Colour $blink $blue $cyan $green $yellow $red $reset $white $magenta $brgreen);
use vars qw($blink $blue $cyan $green $yellow $red $reset $white $magenta $brgreen);
#################################################
...
sub Colour {
  ...
  if ($opt_C) {
    ...
    $reset=color("reset");
    $white=color("bold white on_black");
    $magenta=color("bold magenta on_black");
    $brgreen=color("bold green on_black");
  } else {
    ## Color settings for Term::AnsiColor
